...
*/
    model {
        ...
        auth0tnt = softwareSystem "Auth0 Tenant API" "Authentication API" "New Software System"
        auth0ui = softwareSystem "Auth0 Tenant UI" "Login App" "New Web Browser"
        ...
        srssystem = softwareSystem "SafeRent Platform" {
            ...
            group "SRS" {
                ...
                farapi = container "FAR API service" {
                    technology ".NET, Web API"
                    description "Custom DB Auth0 Interface"
                    tags "API, NewComponent"
                }

                newAccountUI = container "NewAccount" {
                    technology "SPA Angular 17"
                    description "Clients Onboarding"
                    tags "Web Browser"
                }

                obUI = container "Online Billing" {
                    technology "SPA Angular 17"
                    description "Invoice payments Portal"
                    tags "Web Browser"
                }
            }
            ...
            srsweb -> regaccessdb "saves permissions"
            appsmyrental -> regaccessdb "saves permissions"
            regaccessdb -> srsweb "gets permissions"
            regaccessdb -> appsmyrental "gets permissions"
            regaccessdb -> mpb "gets permissions"
            regaccessdb -> srsb2b "gets permissions"

            farapi -> regaccessdb "saves credentials"
            regaccessdb -> farapi "gets credentials"

            srsweb -> auth0ui "gets authentication"
            appsmyrental -> auth0ui "gets authentication"
            srsdyn -> auth0tnt "gets authentication"
            obUI -> auth0ui "gets authentication"
            newAccountUI  -> auth0ui "gets authentication"
            // regaccessdb -> srsweb "gets permissions"
            // regaccessdb -> appsmyrental "gets permissions"
            // regaccessdb -> mpb "gets permissions"
            // regaccessdb -> srsb2b "gets permissions"
            ...
            osdb -> boportal "gets permissions"
            pportal -> osdb "persists permissions"

            boportalui -> auth0ui "verifies authentication"
            ...
            // Auth0 Tenant
            farapi -> auth0tnt "retrieves user profile"
            auth0tnt -> farapi "verifies credentials"
            srsb2b -> auth0tnt "verifies credentials"
            tsui -> auth0ui
            dm -> auth0tnt
            mpf -> auth0ui
        }
